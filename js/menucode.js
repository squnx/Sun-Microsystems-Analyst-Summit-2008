...
function insertmenus(){
		...
		var x = 1;
		var xx = 1;
		while (navmenu[x+'.1'] && document.getElementById("mtopic" + x)){
			checkforoldmenu(x);
			var thismenu =  "";
...
function checkforoldmenu(mnum){
	...
	s = s.replace(/(http:\/\/[^\/]+\/).*/,"$1");
	if (oldmenu[mnum+'.0']){
		var mh = document.getElementById('glink'+mnum).href;
		var ol = oldmenu[mnum+'.0'].split('|');
		if (ol[0] == document.getElementById('glink'+mnum).innerHTML || ol[1] == mh || ol[1] == s+mh || ol[1] == 'http://www.sun.com/'+mh){
			var mm = 0;
