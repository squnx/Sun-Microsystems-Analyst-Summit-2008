...
function k5Click(e) {
	...
	if (hasClassName(this,'k5vid')) { k5DivClass += ' k5vid'; }
	...
	//var close = elem('a',{'href':'#','title':'close'},'X')
	var close = elem('a',{'href':'#'},elem('img',{'title':'close','alt':'close','src':imdir+'/ic_close_win_big_x.gif','border':'0'}))
	var objectDiv = elem('div.k5w2',{},' ');
	objectDiv.refId = id;
	var k5VidImg = hasClassName(this,'k5vid') ? elem('div.k5vidshadow',{},elem('img#k5vidshadow',{'src':'/im/k5_vid_shadow.png','alt':''})) : ' ';
	...
	var topMatch = matchClassName(this,/^top(\d+)$/);
	if (topMatch) { k5Div.firstChild.style.marginTop=topMatch[1]+'px'; if(hasClassName(this,'k5vid')){k5VidImg.style.marginTop=topMatch[1]+'px';} }
...
function sniffA1(fobj){
	...
	for (var a=0;a<alinks.length;a++){
		if (hasClassName(alinks[a], 'language-select')){
			...
			a1divs = a1divs+w0+'languageselector'+a1menuwrap[1]+'<h5></h5><div></div>'+a1menuwrap[2];
			continue;
		}
		...
		for (i in a1hrefs){
			if (a1hrefs[i][0] == thisText[1]){
				...
				if (a1menus[i]){
					...
					var w0 = a1menuwrap[0].replace(/set_outerwidth/,'style="width:'+outw+'px;min-width:'+outw+'px;"');
					a1divs = a1divs+w0+i+a1menuwrap[1]+a1menus[i][1]+a1menuwrap[2];
					a1menus[i][1] = "";
					a1menus[i] = false;
				}
			}
...
function sniffG27(fobj) {
	...
	var mp = (pm == 'p')?'m':'p';
 	fobj.getElementsByTagName('h3')[0].innerHTML = '<a href="#" class="toggleObj objects-g27obj'+g27c+'-g27ttp'+g27c+'-g27ttm'+g27c+'"><span id="g27ttp'+g27c+'" class="g27'+pm+'">&nbsp;</span><span id="g27ttm'+g27c+'" class="hidethis g27'+mp+'">&nbsp;</span> '+fobj.getElementsByTagName('h3')[0].innerHTML+'</a>';
